...
sys.path.append(os.path.join(os.getcwd(), "mmsegmentation"))


@click.group()
def mmseg():
    """CLI for training and managing a MMSeg model on a custom dataset."""
...
def train(config_file: str, load_from: str | None):
    """Train the MMSeg model."""
    MMSegModels.train(config_file=config_file, load_from=load_from)


@mmseg.command()
@click.option("--config-file", type=str, required=True, help="Name of the model")
@click.option("--epoch", type=int, default=-1, help="Epoch which segmentation should be made for")
...
@click.option(
    "--results-file", type=str, default="segmentations.csv", help="Name of the resulting CSV file"
)
@click.option(
...
@click.option(
    ...
    type=float,
    default=0.5,
    help="Minimum confidence score for segmentation",
)
@click.option(
    "--device", type=str, default="cuda:0", help="Device to use for segmentation (default: cuda:0)"
)
def detect(
    config_file,
    epoch,
...
@mmseg.command()
@click.option("--model_type", type=str, required=True, help="Type of model to use for segmentation")
@click.option("--model_name", type=str, required=True, help="Name of the model")
@click.option(
    "--gt-masks-dir",
    type=click.Path(exists=True, file_okay=False, dir_okay=True),
    required=True,
    help="Path to the ground truth masks directory",
)
@click.option(
