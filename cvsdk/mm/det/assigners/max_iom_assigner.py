...
def box_iom(boxes1, boxes2):
    """Compute intersection over minimum between two sets of boxes.
    ...
    Returns:
        Tensor: Shape (n, m) where the value at (i, j) represents
            the IoM (Intersection over Minimum) between boxes1[i] and boxes2[j].
    """
    ...
    # Compute minimum of the two areas
    min_area = torch.min(area1[:, None], area2[None, :])

    # IoM = intersection / min(area1, area2)
    iom = inter / min_area.clamp(min=1e-6)

    return iom


def box_iof(boxes1, boxes2):
    """Compute intersection over foreground (boxes1) between two sets of boxes.

    Args:
        boxes1 (Tensor): Shape (n, 4) in "xyxy" format (foreground/denominator).
        boxes2 (Tensor): Shape (m, 4) in "xyxy" format.

    Returns:
        Tensor: Shape (n, m) where the value at (i, j) represents
            the IoF (Intersection over Foreground) between boxes1[i] and boxes2[j].
    """
    area1 = (boxes1[:, 2] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 1])
    ...
    iof = inter / area1[:, None].clamp(min=1e-6)
...
class MaxIoMAssigner(BaseAssigner):
    """Assign a corresponding gt bbox or background to each bbox using
    Intersection over Minimum (IoM) strategy.
    ...
    IoM (Intersection over Minimum) computes the intersection area divided by
    the minimum of the two box areas. This is more suitable for small objects
    than IoU because it doesn't penalize cases where one box is much smaller
    than the other as heavily.

    Args:
        pos_iou_thr (float): IoM threshold for positive bboxes.
        neg_iou_thr (float or tuple): IoM threshold for negative bboxes.
        min_pos_iou (float): Minimum iom for a bbox to be considered as a
            positive bbox. Positive samples can have smaller IoM than
            pos_iou_thr due to the 4th step (assign max IoM sample to each gt).
            `min_pos_iou` is set to avoid assigning bboxes that have extremely
            small iom with GT as positive samples. It brings about 0.3 mAP
        ...
        iou_calculator (dict): Config of overlaps Calculator. Defaults to
            using box_iom function for Intersection over Minimum.
    ...
    def __init__(self,
        ...
        # Use box_iom by default for Intersection over Minimum
        if iou_calculator is None:
            self.iou_calculator = box_iom
        else:
    ...
    def assign(self,
        ...
        if (self.ignore_iof_thr > 0 and gt_bboxes_ignore is not None
            ...
            # Check if iou_calculator is a callable function (like box_iom)
            # or a mmdet task utils object with 'mode' parameter
            # Functions have __module__ attribute, class instances don't
            is_function = hasattr(self.iou_calculator, '__module__')
            
            if not is_function:
                # It's a mmdet built object, use its mode parameter
                ...
                # It's a function, use box_iof directly
                if self.ignore_wrt_candidates:
                    ignore_overlaps = box_iof(priors, gt_bboxes_ignore)
                    ignore_max_overlaps, _ = ignore_overlaps.max(dim=1)
                else:
                    ignore_overlaps = box_iof(gt_bboxes_ignore, priors)
                    ignore_max_overlaps, _ = ignore_overlaps.max(dim=0)
