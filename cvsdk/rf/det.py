...
# Valid model sizes for RF DETR
MODEL_SIZES = ['nano', 'small', 'base', 'medium', 'large', 'xlarge', '2xlarge']

# Mapping of size strings to RF DETR model classes
DET_MODEL_CLASSES = {
    'nano': RFDETRNano,
    'small': RFDETRSmall,
    'base': RFDETRBase,
...
def train(dataset_dir, size, epochs, batch_size, grad_accum_steps, lr, output_dir, resume, early_stopping):
    ...
    This command initializes an RF DETR model and starts training with the specified parameters.
    """
    click.echo(f"Starting RF DETR training...")
...
@click.option('--output', default='detections.csv', type=click.Path(), help='Output CSV file path (default: detections.csv).')
...
def detect(checkpoint, size, image, output, score_threshold):
    """
    Run object detection on a single image using a trained RF DETR model.
    
    This command loads a model from a checkpoint and runs inference on the specified image.
    """
    click.echo(f"Running RF DETR detection...")
    click.echo(f"Model size: {size}")
    ...
    click.echo(f"Detections saved to {output}")
...
def eval(checkpoint, size, dataset_dir, output, score_threshold):
    ...
    click.echo(f"Total detections: {len(all_detections)}")
...
def export(checkpoint, size, output, input_size):
    ...
    """
    click.echo(f"Exporting RF DETR model to ONNX...")
    click.echo(f"Model size: {size}")
    click.echo(f"Checkpoint: {checkpoint}")
    click.echo(f"Output file: {output}")
