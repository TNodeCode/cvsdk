    ...
    with open(log_file_path, 'r') as f:
        for line in f:
            try:
                ...
                elif any(k.startswith("mIoU") or k.startswith("mAcc") for k in log_entry):
...
def log_metrics_to_tensorboard(batch_df: pd.DataFrame, eval_df: pd.DataFrame, log_dir: str="./runs") -> None:
    ...
    for _, row in batch_df.iterrows():
        ...
        for key, value in row.items():
            if isinstance(value, (int, float)) and key not in ("iter", "epoch", "memory"):
    ...
    for _, row in eval_df.iterrows():
        if "iter" in row:
            ...
            continue
        for key, value in row.items():
            if isinstance(value, (int, float)) and key != "iter":
